...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in actions:
            if (tuple(state), action) not in self.q:
                ...
                possible_q_values.append(self.q[(tuple(state), action)])

        best_possible_reward = max(possible_q_values)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        possible_q_values = dict()
        for action in actions:
            if (tuple(state), action) not in self.q:
                possible_q_values[action] = 0;
            else:
                possible_q_values[action] = self.q[(tuple(state), action)]

        if epsilon == True:
            decision = random.choices([0, 1], weights = [self.epsilon, 1 - self.epsilon])
            if decision[0] == 0:
